...
| `d` | 收→发 | 数据确认 |
| `B` | 任意→广播 | 广播数据 |
| `C` | 新→广播 | 竞选声明（未入网时比随机数大小） |

### 心跳与路由表
心跳数据携带 20 位路由状态（如 `"10110000000000000000"` 表示地址1在线、地址2离线、地址3/4在线），所有节点收到后学习发送节点视角的全网状态。NEIGHBOR_TIMEOUT 无心跳判离线。
...
|------|------|------|
| `S` | 仅字符 | 打印路由表 |
| `D<addr> <data>` | `D2 Hello` | 单播到指定节点（空格分隔） |
| `B<data>` | `BHello All` | 广播数据 |
| `L` | 仅字符 | 退网 |
| `T<addr> <n>` | `T2 10` | Ping 测试（空格分隔） |
| `R` | 仅字符 | 重置统计 |
...
├──────────┬──────────┬──────────┬───────────────────────┤
│  节点A    │  节点B    │  节点C    │  节点D                │
│ [COM][连]  │ [COM][连] │ [COM][断] │ [COM][连]           │
│ [单播]     │ [单播]    │ [单播]    │ [单播]               │
│ [广播]     │ [广播]    │ [广播]    │ [广播]               │
│ [Ping][S/L/R] │ ...   │ ...       │ ...                  │
│ ┌─────────┐ │ ┌────────┐│ ┌────────┐│ ┌────────┐        │
│ │RSSI折线图│ │ │RSSI   ││ │RSSI   ││ │RSSI   │        │
│ │ 实时曲线 │ │ │ 折线图 ││ │ 折线图 ││ │ 折线图 │        │
│ └─────────┘ │ └────────┘│ └────────┘│ └────────┘        │
│  日志框     │  日志框    │  日志框    │  日志框            │
└────────────┴────────────┴────────────┴──────────────────┘
...
- 连接后自动识别节点地址，标题改为 `节点A - N2` 格式
- 路由表实时更新（只显示在线节点，不叠加）
- 网络统计按 key 覆盖更新（不堆积旧值）
- 每个面板内置 RSSI 折线图（0 ~ -50dBm，约 1 分钟窗口）
- RSSI 自动轮询：每 3 秒向已连接节点发广播触发回传 RSSI，可停止
