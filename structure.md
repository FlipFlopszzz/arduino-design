...
| 宏 | 当前值 | 说明 |
|------|-------|------|
| SCAN_LISTEN_TIME | 5000 | 新人监听时长 |
| SCAN_JITTER_MAX | 4000 | 监听随机抖动 |
| CLAIM_WINDOW | 2000 | 竞选窗口 |
| HEARTBEAT_INTERVAL | 2000 | 心跳间隔 |
| NEIGHBOR_TIMEOUT | 8000 | 超时判离线 |
| ACK_TIMEOUT | 500 | ACK 等待超时 |
| RETRY_MAX | 3 | 入网重试次数 |
| RAPID_BEACONS | 5 | 刚入网时快速宣告次数 |
...
| myAddr | byte | 本机地址（0=未分配，1~20=已入网） |
| route[] | RouteEntry[MAX_NODES] | 路由表，下标对应地址-1 |
...
| D | D<addr> <data> | 单播，如 `D2 Hello`（空格分隔） |
| B | B<data> | 广播，如 `BHello All` |
| T | T<addr> <n> | ping 测试，如 `T2 10`（空格分隔） |
| L | 单字符 | 本机退网，之后自动重新入网 |
| R | 单字符 | 重置统计数据 |
...
### 1. 扫描（SCANNING）
- 持续 5~9 秒（5s + ADC 噪声 0~4s 随机抖动）
- **只收不发**，防止干扰
- 听到 `F_HEARTBEAT` + 地址 1~20 → 转 JOINING
...
- 广播 `F_CLAIM` 声明
- 2 秒内比较谁最大：
  - WINNER → 自分配 1，快速心跳 5 次宣告存在
  - LOSER → `canClaim=false`，重新扫描只加入不竞选

### 3. 加入（JOINING）
- 生成随机令牌（1~255）
- 发 `F_JOIN_REQ`（令牌 + "?"）
- 等 500ms，收到 `F_JOIN_ACK`（回传令牌 + 分配地址）
  - **令牌匹配** → 设为分配地址 → 入网
  - **令牌不匹配** → 丢弃（防止两个新节点抢同一个 ACK）
- 500ms 超时 → 重试，3 次失败 → 回扫描

### 4. 入网后（IN_NETWORK）
- 每 2 秒广播心跳，携带路由表
- 收到心跳 → 直接更新 sender 的 lastSeen
- 心跳中携带的路由表 → 间接学习其他节点（刷新 lastSeen，防止信号差被误判离线）
- 8 秒没任何节点收到某节点的心跳 → 判离线

### 5. 心跳（HEARTBEAT）
已入网节点定期宣告存在，格式：

```
| DAddr=0xFF | SAddr=2 | Function='H' | DataLen=20+'0' | MCS=? | MsgData="11100000000000000000" |
```

MsgData 的第 i 位 = 地址 i+1 的在线状态，最多 20 位：
- `"11100000000000000000"` = 节点 1/2/3 在线，其余离线
...
### 心跳机制
- 已入网节点每 2 秒广播一次心跳
- 心跳携带**本机视角的路由表**（MAX_NODES 位，每位对应一个节点在线状态）
- 刚入网时 5 次快速宣告（100ms 间隔），让大家立刻知道新节点

### 超时判离线
- 每个节点记录路由表中其他节点的 lastSeen
- 任意节点 8 秒内没有被**任何人**直接或间接听到 → 判离线
